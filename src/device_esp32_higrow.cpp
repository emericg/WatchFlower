...
void DeviceEsp32HiGrow::serviceDetailsDiscovered(QLowEnergyService::ServiceState newState)
{
    if (newState == QLowEnergyService::ServiceDiscovered)
    {
        ...
        if (serviceData)
        {
            ...
            QBluetoothUuid b(QString("beb5483e-36e1-4688-b7f5-ea07361b26a2")); // battery
            ...
            QBluetoothUuid rt(QString("0d563a58-196a-48ce-ace2-dfec78acc814")); // rt data
...
*/
    if (c.uuid().toString() == "{0d563a58-196a-48ce-ace2-dfec78acc814}")
    {
        // HiGrow realtime data // handler 0x?

        if (value.size() == 16)
        {
            m_temp = static_cast<uint16_t>(data[0] + (data[1] << 8)) / 10.f;
            m_humi = data[2];
            m_hygro = data[3];
            m_conductivity = static_cast<uint16_t>(data[4] + (data[5] << 8));
